...
[![](Figures/InferenciaASVs.png){width="938"}](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/1471-2105-13-283  https://resjournals.onlinelibrary.wiley.com/doi/10.1111/syen.12406)
...
# copia los datos
cp -r /home/alumno1/taller_amplicones_pozol/data .
```
...
``` bash
nano src/create_manifest.sh
```

Copia y pega el siguiente contenido en \`src/create_manifest.sh\` :
...
``` bash
qiime tools export --input-path results/03.denoising-stats_v1.qza --output-path results/02.qiime/03.denoising-stats_v1
```
...
otu_table_ampvis <- data.frame(OTU = rownames(phyloseq::otu_table(ps)@.Data),
                               phyloseq::otu_table(ps)@.Data,
                               phyloseq::tax_table(ps)@.Data,
                               check.names = FALSE)
meta_data_ampvis <- data.frame(phyloseq::sample_data(ps),
                               check.names = FALSE
)
